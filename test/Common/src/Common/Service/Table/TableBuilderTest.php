...
class TableBuilderTest extends MockeryTestCase
{
    ...
    public function testRenderBodyColumnWithFormatterAndActionType()
    {
        ...
        $expected = '<button data-prevent-double-click="true" data-module="govuk-button" type="submit" class="" name="action[edit][1]" >' . date('d/m/Y') . '</button>';
        $mockContentHelper->expects($this->once())
            ->method('replaceContent')
            ->with(
                '{{[elements/td]}}',
    ...
    public function testRenderBodyColumnWithActionType()
    {
        ...
        $mockContentHelper->expects($this->once())
            ...
            ->with(
                '{{[elements/td]}}',
                [
                    'content' => '<button data-prevent-double-click="true" data-module="govuk-button" type="submit" class="" name="action[edit][1]" >bar</button>',
    ...
    public function testRenderBodyColumnWithActionTypeAndFieldset()
    {
        ...
        $mockContentHelper->expects($this->once())
            ...
            ->with(
                '{{[elements/td]}}',
                [
                    'content' => '<button data-prevent-double-click="true" data-module="govuk-button" type="submit" class="" name="table[action][edit][1]" >bar</button>',
                    'attrs' => ' class="' . TableBuilder::CLASS_TABLE_CELL . '"',
                ]
            );
