...
foreach ($commonTranslations as $langKey => $translations) {
    ...
    foreach ($mergedTranslations as $langKey => $translations) {
        if($langKey != 'en_GB') {
            foreach ($translations as $transKey => $translatedText) {
                if (array_key_exists($transKey, $mergedTranslations['en_GB']) && $mergedTranslations['en_GB'][$transKey] == $translatedText) {
